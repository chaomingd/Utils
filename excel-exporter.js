...
class ExcelExporter {
  styleKeys = ['numFmt', 'font', 'alignment', 'border', 'fill'] // 样式属性
  ...
  calcHeaderDepth (columns) { // 计算列表头的深度  占多少行 并返回colmns的克隆对象
    const depths = []
    ...
    const newColumns = workder(columns, 1)
  ...
  exportExcel ({ sheetHeader, dataRowsResult, fileName }, done) { // 写入excel数据并导出
  ...
  correctHeaderCells(treeWidth, sheetHeader) { // 修正表头的colSpan 和 colIndex
    sheetHeader.cells.forEach(headerCell => {
    ...
    sheetHeader.dataItems.forEach(dataItem => { // 修正dataItem的colIndex
      if (dataItem.colIndex === 1) return
      dataItem.colIndex += treeWidth - 1
    })
  }
  renderDataSource (dataRowsResult, sheetHeader) { // 渲染数据
    const worksheet = this.worksheet
    const startRow = sheetHeader.headerEndRow + 1
    const dataItems = sheetHeader.dataItems
    const _this = this
    dataRowsResult.dataRows.forEach((dataRowItem, index) => {
  ...
  setCellStyle (cell, style) { // 设置单元格样式
  ...
  setColStyle (col, colWidth, colStyle) { // 设置列样式
    const styleKeys = this.styleKeys
  ...
  setHeaderGroupStyle (colItem) { //表头分组默认居中显示
    if (!colItem.style) {
      colItem.style = {}
    }
  ...
  resolveDataSource (data) { // 处理数据， 生成每行数据的开始列号、深度、及最大深度等信息
    const dataRows = []
    let maxStartColIndex = 1
    const _this = this
    function worker(data, parents) { // 支持树形结构
      data.forEach(item => {
  ...
  resolveRowHeader (columns, rowDepth) { // 处理表头， 输出一维数组， 包含每个表头的rowIndex, colIndex, colSpan, rowSpan以及样式
    const results = []
    const dataItems = []
    const _this = this;
    function worker (columns, rowSpan, parents) {
      columns.forEach((col, index) => {
        ...
        if (col.children) {
          ...
          colItem.children = worker(col.children, rowSpan - 1, newParents)
        } else {
